...
function calculate() {
  var myPhrase = document.getElementById("message").value;
  var possibleCharacters = "";
  let characters = [];
  let frequency = [];
  for (var i = 32; i <= 126; i++) {
    possibleCharacters += String.fromCharCode(i);
  }
  console.log(possibleCharacters);
  for (var i = 0; i < possibleCharacters.length; i++) {
    for (var j = 0; j < myPhrase.length; j++) {
      if (possibleCharacters[i] == myPhrase[j]) {
        var exists = false;
        var position = 0;
        for (var k = 0; k < characters.length; k++) {
          if (possibleCharacters[i] == characters[k]) {
            exists = true;
            position = k;
          }
        }
        if (exists) {
          ...
          frequency.push(1);
          characters.push(possibleCharacters[i]);
        }
      }
    }
  }
  for (var i = 0; i < frequency.length; i++) {
    for (var j = i + 1; j < frequency.length; j++) {
      if ((frequency[j] > frequency[i])||((frequency[j] == frequency[i])&&(characters[j]<characters[i]))) {
        var aux = frequency[i];
